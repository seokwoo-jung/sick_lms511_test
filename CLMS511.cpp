...
void CLMS511::DataRecv()
{
    ...
    if(telegrem_buff_parse.size() == (unsigned int)(lms_data_num+31))
    {
        ...
        mpc_pcl->cloud->points.resize(lms_data_num);
        ...
        for(int j= 26;j<= (26+lms_data_num-1);j++)
        {
            ...
            mpc_pcl->cloud->points[point_index].x = (double)distance*0.001*cos(current_angle);
            mpc_pcl->cloud->points[point_index].y = (double)distance*0.001*sin(current_angle);
            mpc_pcl->cloud->points[point_index].z = 0;
            mpc_pcl->cloud->points[point_index].r = 255;
            mpc_pcl->cloud->points[point_index].g = 255;
            mpc_pcl->cloud->points[point_index].b = 255;

            if(max_y < mpc_pcl->cloud->points[point_index].y)
            {
                max_y = mpc_pcl->cloud->points[point_index].y;
            }
        }
    ...
    emit SignalLMS511UpdatePCL(parse_complete);
}
